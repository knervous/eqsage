...
const _reinterpretBuf = new DataView(new ArrayBuffer(8));
let logLine = [];
...
class Go {
  fileSystem = {
    ...
    read(fd, buffer, offset, length, position, callback) {
      try {
        ...
        const pos = position !== null ? position : fileHandle.position;
  ...
  run(instance) {
    ...
    this._inst = instance;
    this.mem = new DataView(this._inst.exports.mem.buffer);
    globalThis.fs = this.fileSystem; // eslint-disable-line no-undef
    globalThis.process = { // eslint-disable-line no-undef
      cwd() {
        return '/';
      },
      chdir() {},
    ...
    this._ids = new Map([
      ...
      [true, 3],
      [false, 4],
      [globalThis, 5], // eslint-disable-line no-undef
      [this, 6],
    ]);
