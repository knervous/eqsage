...
export const ExportS3DDialog = ({ open, setOpen }) => {
  ...
  const doExport = useCallback(async () => {
        ...
        imageWritePromises.push(
          new Promise((res) =>
            compressPNG(data).then((buffer) => {
              ...
              eqgArchive.setFile(convertedName, new Uint8Array(buffer));
              res();
            })
          )
        );
    ...
    const writeEqgMod = async (
      ...
      if (terrain) {
        const zoneTextures = zb.zoneContainer
          .getChildMeshes()
          .flatMap((m) => m?.material?.getActiveTextures() ?? []);
      ...
      {open && (
        <>
          ...
          <Typography
              ...
              gutterBottom
            >
              Note: This feature is in development and currently does not work, only the client aspects are functional
            </Typography>
          <Typography
            sx={{
              fontSize: 16,
