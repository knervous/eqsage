...
export const useProject = () => {
  ...
  const updateMetadata = useCallback(fn => {
  ...
  const saveProject = useCallback(async () => {
    const encoded = msgpack.encode(gameController.ZoneBuilderController.project);
    const zipped = pako.deflate(encoded);
    await writeEQFile('projects', gameController.ZoneBuilderController.project.projectName, zipped);
    if (alert) {
      openAlert(`Project ${gameController.ZoneBuilderController.project.projectName} successfully saved.`);
    }
  }, [openAlert]);
