...
function range(start = 0, end, step = 1) {
  ...
  // If only one argument is provided, shift it as the 'end', and start from 0
  if (end === undefined) {
    end = start;
    start = 0;
  }

  // Adjust step for descending ranges
  if (step === 0) {
...
export class MIDIPlayer extends Player {
  ...
  ensureWebMidiInitialized() {
    if (this.webMidiIsInitialized === true) {
      return;
    }
  ...
  async loadData(data, filepath) {
    ...
    //   const useMelodicChannel10 = false;
    ...
    this.activeChannels = [];
    for (let i = 0; i < 16; i++) {
      if (this.midiFilePlayer.getChannelInUse(i)) {
        this.activeChannels.push(i);
      }
    }
  ...
  getParameter(id) {
    if (id === 'fluidpoly') {
      return core._tp_get_polyphony();
    }
    if (this.transientParams[id]) {
      return this.transientParams[id];
    }
